            ...
            if($resultProducts->num_rows> 0){
                ...
                while($row= $resultProducts->fetch_assoc()){
                    ...
                    if ($row["doubleProductPrice"]) {
                        echo "<input type='radio' name='" . $row["productName"] . "SinglePrice' id='" . $row["productName"] . "Price1' value='" . $row["singleProductPrice"] . "' onclick=\"handleRadio('" . $row["productName"] . "', this);setPrice('".$row["productName"]."Price1','".$row["productName"]."Qty','".$row["productName"]."Total')\" /><strong>Single ".number_format($row["singleProductPrice"], 2)."</strong>";
                        echo "<input type='radio' name='" . $row["productName"] . "DoublePrice' id='" . $row["productName"] . "Price2' value='" . $row["doubleProductPrice"] . "' onclick=\"handleRadio('" . $row["productName"] . "', this);setPrice('".$row["productName"]."Price2','".$row["productName"]."Qty','".$row["productName"]."Total')\"/><strong>Double ".number_format($row["doubleProductPrice"], 2)."</strong>";
                    } else {
                    ...
                    echo "<td>Quantity: <input type='number' name='".$row["productName"]."Qty' id='".$row["productName"]."Qty' min='0' value='0' placeholder='Enter the quantity' required onblur=\"validateInput(this)\" onchange=\"calculateSubTotal('".$row["productName"]."Qty','".$row["productName"]."Total')\"/></td>";
